...
class ventana():
    ...
    def crear_nueva_ventana(self):
        self.vnt=ventana_nueva("400x220", "INICIALIZADOR DE OPERACIONES", self.wn)
        self.vnt.datos()
        self.vnt.mainloop()
...
class ventana_nueva (ventana):
    def __init__(self, geo="800x500", tl="SISTEMA DE CONTROL VEHICULAR", ventana_madre=None, icon="./images/Micro.ico"):
        self.wn=tk.Toplevel()
        self.wn.geometry("800x500")
    ...
    def datos(self):
        ...
        boton(self.wn, 50, 170, "ENVIAR", "grey", cmd=lambda:self.enviar_dato(self.hora.get())).medida(100)
        boton(self.wn, 250, 170, "FINALIZAR", "grey", cmd=self.wn.destroy).medida(100)
